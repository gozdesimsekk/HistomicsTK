...
# Veriyi oku
data = pd.read_csv('cluster_mmc2.csv')
...
categorical_vars = ['path_diagnosis', 'idh1_r132h_mut_status', 'mgmt_methyl_status', 
                   'tumor_site', 'tumor_laterality', 'sample_type', 'who_tumor_grade']

# Her kategorik değişken için görselleştirme ve analiz
for var in categorical_vars:
    ...
    plt.tight_layout()
    plt.savefig(f'cluster_{var}_distribution_all1.png', bbox_inches='tight', dpi=300)
    plt.close()
    
    # Not Available ve Not Performed olmayan verileri filtrele
    valid_data = temp_data[~temp_data[var].isin(['Not Available', 'Not Performed'])]
    
    if len(valid_data) > 0:  # Eğer geçerli veri varsa
        ...
        plt.legend(title=var, bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig(f'cluster_{var}_distribution_valid1.png', bbox_inches='tight', dpi=300)
        plt.close()
        
        # Yüzdesel dağılım görselleştirmesi (sadece geçerli verilerle)
        plt.figure()
        ...
        # Yüzde değerlerini göster
        for c in ax.containers:
            ax.bar_label(c, fmt='%.1f%%', label_type='center')
        
        plt.tight_layout()
        plt.savefig(f'cluster_{var}_percentage_valid1.png', bbox_inches='tight', dpi=300)
        plt.close()
...
# Sonuçları yazdır
with open('cluster_analysis_results1.txt', 'w', encoding='utf-8') as f:
    f.write('Klinik Değişkenlerin Cluster\'larla İlişkisi - İstatistiksel Analiz\n')
    f.write('=========================================================\n\n')
...
print("Analiz tamamlandı. Görsel ve istatistiksel sonuçlar kaydedildi.")
print("Her değişken için hem tüm verilerle hem de sadece geçerli verilerle analiz yapıldı.")
print("Detaylı istatistiksel sonuçlar 'cluster_analysis_results.txt' dosyasına kaydedildi.") 